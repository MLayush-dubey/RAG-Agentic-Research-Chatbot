FROM python:3.11-slim

WORKDIR / app

#Install system dependencies
RUN apt-get update && apt-get install -y \
    build-essential \
    && rm -rf /var/lib/apt/lists/*

#copy requirements and install \
COPY requirements.txt ./
RUN pip install --no-cache-dir -r requirements.txt


#copy source code
COPY src/ src/
COPY documents/ documents/
COPY start.sh ./start.sh

#Make start.sh executable
RUN chmod +x start.sh

#expose backend and frontend
EXPOSE 8000 8501

#set env variables(can be overriden any time)
ENV GROQ_API_KEY="your_GROQ_api_key"
ENV DOCUMENTS_DIR="path/to/documents"
ENV VECTOR_STORE_DIR="path/to/doc_vector_store"
ENV COLLECTION_NAME="document_collection"
ENV MODEL_NAME="llama-3.1-8b-instant"
ENV MODEL_TEMPERATURE=0.0
ENV CHAT_ENDPOINT_URL="http://localhost:8000/chat/answer"


#run all services using start.sh
CMD ["app/start.sh"]